...
# Get the convex hull containing the excluded comunities
excluded_hull <- gConvexHull(excluded_communities)
...
We want to sample `r cluster_size` households within each cluster, which considering
a `r signif(response*100, 3)`% of response need to have *at least*
...
This aggregation results in cells of roughly `r signif(res(cluster_cells)[1], 1)`
...
randomly select `r adj_clusters` clusters with probability proportional to size.
