...
# Sampling design for HAPIN scoping survey
...
```{r geo-limits, results='hold', dpi=dpi, fig.width=9, fig.height=3}
# Arrange plots
dpar <- par()
par(mfrow = c(1, 3))
...
```{r pop-density, results='hold', dpi=dpi, fig.width=9, fig.height=6.5}
# Raw density
dpar <- par()
par(mar = c(2, 2, 3, 1))
plot(study_density)
...
```{r clusters-density, dpi=dpi, fig.width=9, fig.height=6.5}
dpar <- par()
par(mar = c(2, 2, 3, 1))
plot(cluster_cells)
...
```{r clusters-services, dpi=dpi, fig.width=9, fig.height=6.5}
dpar <- par()
par(mar = c(2, 2, 3, 1))
# Base map
plot(jalapa, asp = 1)
